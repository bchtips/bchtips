...
function refreshData(){
	...
	// queued tx
	chrome.storage.largeSync.get(['tx_queue','tx_sent','tx_attempts'/*,'txq_lastrun'*/],function(o){
		...
		if(o && o.tx_queue && o.tx_queue.length>0){
			...
			for(var i=o.tx_queue.length-1;i>=0;i--){
				...
				var row='<tr id="txqr'+o.tx_queue[i][0]+'"><!--<td>'+s+'</td>-->'
				+'<td>'+d+'</td>'
	...
	chrome.storage.sync.get(['data','fee'],function(df){
		...
		chrome.storage.largeSync.get(['tx_queue','tx_attempts'],function(o){
			...
			if(o && o.tx_queue && o.tx_queue.length>0){
				...
				sendQueued([df,o,item],function(cb){
...
function hideQueued(id){
	if(document.getElementById('txqr'+id)) document.getElementById('txqr'+id).style.display='none';
}